...
from Challenge18_Implement_CTR_the_stream_cipher_mode import implementAES_CTR
from Challenge03_Single_byte_XOR_cipher import breakSingleByteXor
...
def main():
    ...
    encrypted = [implementAES_CTR(b64decode(x), key) for x in strings]
    # transpose[0] = first byte of each encrypted , and so on
    transpose = list(zip(*encrypted))

    # I can recover part of keyStream
    keyStream = b''
    for i in range(len(transpose)):
        keyStream += bytes([breakSingleByteXor(bytes(transpose[i]))])

    # recover complete keyStream , can be done by manual (adjust by English words)
    # I use plaintext to fill the keyStream here
    keyStream += strxor(encrypted[37][len(transpose):], b'anged in his turn,')
    for i in range(40):
        print(strxor(keyStream[:len(encrypted[i])], encrypted[i]))
